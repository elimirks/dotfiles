...
** Handy functions
   #+begin_src emacs-lisp :tangle yes
     ...
     (defun eli/match-in-current-buffer (regex &optional match-number)
       ...
       (unless match-number (setq match-number 0))
       (let ((result nil))
         (save-excursion
           (goto-char (point-min))
           (while (and (not (eobp)) (not result))
             (let* ((lb (line-beginning-position))
               ...
               (setq result (when (string-match regex ln)
                              (match-string match-number ln)))
               (forward-line 1))))
         result))
   #+end_src
** Global keybindings
...
** Change cursor blink color
   Taken from https://stackoverflow.com/questions/4642835/how-to-change-the-cursor-color-on-emacs
   
   This is a fun little mod to change the colour of the cursor for
   each blink.
...
* Plugins
  ...
  [[https://oremacs.com/2015/05/22/define-word/][Define Word.]] For word lookups.
  ...
  [[https://github.com/storvik/emacs-lastpass/tree/e07b1a062153b9d56d0112ac45caf76d6bce67c5][LastPass]].
  #+begin_src emacs-lisp :tangle yes
...
** LaTeX
   ...
   #+end_src
** MacOS
   Fixes a hickup on MacOS when running in terminal Emacs
   #+begin_src emacs-lisp :tangle yes
     (when (eq system-type 'darwin)
       (lambda ()
         (evil-define-key (kbd "TAB") nil)
...
* AZ AWS Mode
  ...
  #+begin_src emacs-lisp :tangle yes
    ...
    (defvar az-aws/athena/invalid-query-prefix
      "Invalid Query: ")

    (defun az-aws/athena/result-response-to-table (response-json)
      (let* ((raw-rows (cdr (assoc 'Rows (cdr (assoc 'ResultSet query-result)))))
             (rows (mapcar
                    (lambda (raw-row)
    ...
    (defun az-aws/athena/wait-for-query-result (query-id)
      (let ((result nil)
            (response nil))
        (while (not result)
          (sleep-for 2)
          (setq response (az-aws/athena/get-query-result query-id))
          (setq result
    ...
    (defun org-babel-execute:sql-athena (body params)
      ...
      (let* ((session (org-babel-python-initiate-session
             ...
             (query-id-response (az-aws/athena/execute-query body))
             (query-result
              (if (string-prefix-p az-aws/athena/invalid-query-prefix
                                   query-id-response)
                  query-id-response ;; Invalid query, dump the response
                (az-aws/athena/wait-for-query-result query-id-response)))
             (org-output (if (string-prefix-p az-aws/athena/invalid-query-prefix
                                              query-result)
                             query-result ;; Invalid query, dump string
                           (az-aws/athena/result-response-to-table query-result))))
    ...
    (defun az-aws/athena/get-query-result (query-id)
      (let* ((query-response (shell-command-to-string
                              (concat "aws athena get-query-results "
                                      "--query-execution-id "
                                      (shell-quote-argument query-id))))
             (is-valid-query (az-aws/validate-json-string query-response)))
        (if is-valid-query
            (json-read-from-string query-response)
          (concat az-aws/athena/invalid-query-prefix query-response))))

    (defun az-aws/athena/execute-query (query)
      (let* ((query-response (shell-command-to-string
    ...
    (defun az-aws/validate-json-string (string)
    ...
    (defun az-aws/glue/start-crawler (&optional name)
      (interactive)
    ...
    (defun az-aws/s3/ls-recursive (bucket key)
      ...
      (let* ((path (format "s3://%s/%s" bucket key))
        ...
        (mapcar (lambda (entry)
                  ;; TODO: Trim the key string
                  (string-match "^[-0-9]+ [:0-9]+\s+[0-9]+ \\(.*\\)" entry)
                  (substring (match-string 1 entry) (+ 1 (length key))))
                entries)))

    (defun az-aws/s3/open-s3-file (s3-path)
      (let* ((destination "/tmp/emacs-az-aws/")
             (local-file-name
              (progn
                ;; Download the file
                ...
                (string-trim
                 (shell-command-to-string (format "ls %s/ | head -1" destination)))))
             (local-path (concat destination local-file-name)))

        (switch-to-buffer (get-buffer-create "*az-aws-s3-file*"))
        (erase-buffer)
    ...
    (defun az-aws/s3/ivy-open-file-search (bucket key)
      (ivy-read "File to open: "
                (az-aws/s3/ls-recursive bucket key)
                :sort t
                :action (lambda (selection)
                          (az-aws/s3/open-s3-file
                           (format "s3://%s/%s/%s" bucket key selection)))))

    (defun az-aws/emr/normalize-cluster-listing (json)
    ...
    (defun az-aws/emr/ls-recent ()
      (let* ((two-days-ago (format-time-string "%Y-%m-%dT%T" (- (float-time) (* 48 60 60))))
        ...
        (mapcar 'az-aws/emr/normalize-cluster-listing json-cluster-vector)))

    (defun az-aws/emr/describe-cluster (cluster-id)
    ...
    (defun az-aws/emr/list-steps (cluster-id)
      (let* ((output (shell-command-to-string
    ...
    (defun az-aws/emr/get-s3-log-bucket (cluster-id)
      "Returns the bucket,key pair of the s3 bucket log for the given cluster"
      (let* ((cluster-json (az-aws/emr/describe-cluster cluster-id))
    ...
    (defun az-aws/emr/ivy-show-step-logs (cluster-id)
      (let* ((log-bucket (az-aws/emr/get-s3-log-bucket cluster-id))
             (bucket (car log-bucket))
             (log-key (cadr log-bucket))
             (steps-key (format "%s/%s/steps" log-key cluster-id)))
        (az-aws/s3/ivy-open-file-search bucket steps-key)))

    (defun az-aws/emr/ivy-cluster-op (cluster-id)
      (ivy-read (format "Operation (%s): " cluster-id)
                '(("Copy ID to Clipboard" kill-new)
                  ("Dump step logs" az-aws/emr/ivy-show-step-logs))
                :sort t
                :action (lambda (selection)
                          (funcall (cadr selection) cluster-id))))

    (defun az-aws/emr/ivy-ls ()
      (interactive)
      (ivy-read "Cluster: "
                (az-aws/emr/ls-recent)
                :sort t
                :action (lambda (selection)
                          (az-aws/emr/ivy-cluster-op (cadr selection)))))
  #+end_src

  #+begin_src emacs-lisp :tangle yes
    (global-set-key (kbd "C-c a e") 'az-aws/emr/ivy-ls)
  #+end_src
